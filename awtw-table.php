 ...
 */
class Awtw_Log_Table extends WP_List_Table {
    ...
    function column_ip_address($item) {
       // $actions = array('text'=>'Lookup IP Address');
        return sprintf('<a href="http://geomaplookup.net/?ip=%1$s" target="_blank">%1$s</a>', $item->ip_address);
    }
    ...
    function column_feedback($item) {
       ...
       if( isset($_GET['sub']) && $_GET['sub'] != 'spams' || !isset($_GET['sub'])) {
        ...
        //     'mark_unapprove'      => sprintf('<a href="?page=%s&action=%s&feedback=%s" data-action="mark_spam" data-id="%3$s">Mark as Spam</a>',$_REQUEST['page'],'spam',$item->id),
        //     'delete'    => sprintf('<a href="?page=%s&action=%s&feedback=%s" data-action="delete" data-id="%3$s">Delete</a>',$_REQUEST['page'],'delete',$item->id),
        // );
        return sprintf('<p>%1$s</p> %2$s', $item->feedback, $this->row_actions($actions));
    }

    function column_source_url($item) {
        $actions = array('agent' => $item->agent);

        return sprintf('%1$s %2$s', $item->source_url, $this->row_actions($actions));
    }
    function column_created_at($item) {
        return sprintf('%1$s', $item->created_at);
    }
    /** ************************************************************************
     ...
     **************************************************************************/
    function prepare_items($input = 'all') {
        global $wpdb; //This is used only if making any database queries

      //  $tablename = $wpdb->prefix. 'awtw_plugin';
        $alldata = Awtw_DB::getApproved();

        if ($input == 'spams') {
            $alldata = Awtw_DB::getSpam();
        } elseif ($input == 'pending') {
            $alldata = Awtw_DB::getPending();
        }
        ...
        $per_page = $options['awtw_num_entry'];
